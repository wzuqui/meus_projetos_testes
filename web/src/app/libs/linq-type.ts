...
export class List<T = any> {
  ...
  public GroupBy(grouper: (key: T) => any, mapper: (element: T) => any): any {
    return this.Aggregate(
      (ac, v) => (
        (ac as any)[grouper(v)] ? (ac as any)[grouper(v)].push(mapper(v)) : ((ac as any)[grouper(v)] = [mapper(v)]), ac
      ),
      {}
    );
  }
   ...
   */
  public Single(predicate?: (value?: T, index?: number, list?: T[]) => boolean): T {
   ...
   */
  public SingleOrDefault(predicate?: (value?: T, index?: number, list?: T[]) => boolean): T {
...
class ComparerHelper {
  ...
  public static Compare<T>(a: T, b: T, _keySelector: (key: T) => any, descending?: boolean): number {
    ...
    let aA = sortKeyA.replace(reA, "");
    let bA = sortKeyB.replace(reA, "");

    if(aA === bA) {
        var aN = parseInt(sortKeyA.replace(reN, ""), 10);
        var bN = parseInt(sortKeyB.replace(reN, ""), 10);
